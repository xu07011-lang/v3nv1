...
class LogicEngineVipPro:
    ...
    def analyze_and_select(self, issue_id, consecutive_losses=0):
        ...
        # === YÊU CẦU: TÌM NHÂN VẬT ÍT THẮNG NHẤT (TOP 6) TRƯỚC ===
        ...
        # `least_frequent_char` giờ đã được set (là nhân vật bị cấm cược chống lại)
        ...
        # Tìm nhân vật thắng ÍT NHẤT trong lịch sử. (Sử dụng biến đã tính)
        # Lý luận: Nhân vật ít thắng nhất có thể là "kẻ yếu" của game.
        if least_frequent_char != -1:
            # Logic vẫn bỏ phiếu, nhưng sẽ bị lọc ở cuối
        ...
        # === BỘ LỌC GHI ĐÈ (THEO YÊU CẦU) ===
        # KHÔNG BAO GIỜ được cược chống lại nhân vật "ít thắng nhất" (top 6).
        if final_choice == least_frequent_char and least_frequent_char != -1:
            # Lựa chọn hiện tại (final_choice) đã vi phạm quy tắc.
            # Cần tìm lựa chọn tốt thứ hai.
            ...
            for char, votes in ranked_candidates:
                if char != least_frequent_char:
                    # Tìm thấy ứng cử viên tốt nhất tiếp theo mà không vi phạm
                    final_choice = char 
                    found_alternative = True
                    break
            
            if not found_alternative:
                # Trường hợp hiếm: Chỉ có 1 ứng cử viên VÀ đó là NV bị cấm
                # Hoặc không có ứng cử viên nào.
                # Chọn ngẫu nhiên một NV *không phải* là NV bị cấm.
                fallback_options = [c for c in ALL_NV_IDS if c != least_frequent_char]
                if fallback_options:
                    final_choice = random.choice(fallback_options)
                else:
                    # Gần như không thể xảy ra (trừ khi game chỉ có 1 NV)
                    final_choice = random.choice(ALL_NV_IDS)
        ...
        if final_choice not in claimed_chars:
            # Lựa chọn tốt nhất của chúng ta (sau khi lọc) chưa ai chọn -> Claim
            ...
            for char, votes in ranked_candidates:
                # Bỏ qua NV bị cấm (lọc lần 2) VÀ NV đã bị claim
                if char != least_frequent_char and char not in claimed_chars:
                    # Tìm thấy lựa chọn tốt tiếp theo chưa bị claim và không bị cấm
                    self.state_manager.claim_bet(issue_id, char)
                    return char
            
            # Nếu TẤT CẢ các ứng cử viên có phiếu bầu đều đã bị claim HOẶC bị cấm
            # Chọn một nhân vật bất kỳ chưa bị claim VÀ không bị cấm
            unclaimed_fallback = [c for c in ALL_NV_IDS if c not in claimed_chars and c != least_frequent_char]
            if unclaimed_fallback:
                ...
                # Trường hợp cực hiếm: Cả 6 NV đều bị claim hoặc 5 NV bị claim + 1 NV bị cấm.
